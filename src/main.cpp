    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          *
          */
          Eigen::VectorXd dx(ptsx.size());
          Eigen::VectorXd dy(ptsy.size());

          for(int i=0;i<ptsx.size();i++)
          {
            dx[i] = (ptsx[i]-px)*cos(-psi) - (ptsy[i]-py)*sin(-psi);
            dy[i] = (ptsx[i]-px)*sin(-psi) + (ptsy[i]-py)*cos(-psi);
          }
